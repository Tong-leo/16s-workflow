...
def count_sequences_in_fasta_files(input_directory, output_file_path, threshold):
    ...
    for filename in os.listdir(input_directory):
        if filename.endswith(".fasta"):
            ...
            if sequence_count >= threshold:
                shutil.copy(file_path, new_directory)  # 移动文件而不是删除
                moved_files += 1
            else:
    ...
    with open(args.output_file_path + "/bbmap_statistic.txt", 'w') as output_file:
    ...
    plt.title("fasta files sequence count distribution (sequence count > 1)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process and filter fasta files")
    ...
    count_sequences_in_fasta_files(args.input_directory, args.output_file_path, args.threshold)
